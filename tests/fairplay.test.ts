import {
  assert,
  describe,
  test,
  clearStore,
  beforeAll,
  afterAll
} from "matchstick-as/assembly/index"
import { Address, BigInt } from "@graphprotocol/graph-ts"
import { AddUser } from "../generated/schema"
import { AddUser as AddUserEvent } from "../generated/fairplay/fairplay"
import { handleAddUser } from "../src/fairplay"
import { createAddUserEvent } from "./fairplay-utils"

// Tests structure (matchstick-as >=0.5.0)
// https://thegraph.com/docs/en/developer/matchstick/#tests-structure-0-5-0

describe("Describe entity assertions", () => {
  beforeAll(() => {
    let player_addr = Address.fromString(
      "0x0000000000000000000000000000000000000001"
    )
    let name = "Example string value"
    let dob = BigInt.fromI32(234)
    let gender = 123
    let location = BigInt.fromI32(234)
    let uri = "Example string value"
    let newAddUserEvent = createAddUserEvent(
      player_addr,
      name,
      dob,
      gender,
      location,
      uri
    )
    handleAddUser(newAddUserEvent)
  })

  afterAll(() => {
    clearStore()
  })

  // For more test scenarios, see:
  // https://thegraph.com/docs/en/developer/matchstick/#write-a-unit-test

  test("AddUser created and stored", () => {
    assert.entityCount("AddUser", 1)

    // 0xa16081f360e3847006db660bae1c6d1b2e17ec2a is the default address used in newMockEvent() function
    assert.fieldEquals(
      "AddUser",
      "0xa16081f360e3847006db660bae1c6d1b2e17ec2a-1",
      "player_addr",
      "0x0000000000000000000000000000000000000001"
    )
    assert.fieldEquals(
      "AddUser",
      "0xa16081f360e3847006db660bae1c6d1b2e17ec2a-1",
      "name",
      "Example string value"
    )
    assert.fieldEquals(
      "AddUser",
      "0xa16081f360e3847006db660bae1c6d1b2e17ec2a-1",
      "dob",
      "234"
    )
    assert.fieldEquals(
      "AddUser",
      "0xa16081f360e3847006db660bae1c6d1b2e17ec2a-1",
      "gender",
      "123"
    )
    assert.fieldEquals(
      "AddUser",
      "0xa16081f360e3847006db660bae1c6d1b2e17ec2a-1",
      "location",
      "234"
    )
    assert.fieldEquals(
      "AddUser",
      "0xa16081f360e3847006db660bae1c6d1b2e17ec2a-1",
      "uri",
      "Example string value"
    )

    // More assert options:
    // https://thegraph.com/docs/en/developer/matchstick/#asserts
  })
})
